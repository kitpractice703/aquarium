...
(※ 이곳에 메인 화면 스크린샷이나 시연 GIF를 넣어주세요. 시각적 자료는 필수입니다!)
...
배포 URL: 배포된 링크가 있다면 여기에 입력

2. 기획 의도 및 주요 기능
단순한 정보 제공 사이트를 넘어, 사용자가 **몰입감 있는 경험(VR 체험, 먹이 주기 등)**을 예약하고, 커뮤니티를 통해 경험을 공유할 수 있는 플랫폼을 구축하고자 했습니다.
...
5. 트러블 슈팅 (Troubleshooting) 🔥 [포트폴리오 핵심]
...
해결: Spring Boot의 application.properties에서 환경 변수 치환 문법을 활용.

${DB_URL:기본값} 문법을 사용하여, 로컬에서는 IDE 환경 변수로 DB_URL을 주입받고, 배포 서버에서는 기본값(RDS 주소)을 사용하도록 구성하여 코드 수정 없는 배포 파이프라인 구축.
...
Backend: UserService에서 로그인 Provider(local vs google)를 확인하여, 소셜 회원은 비밀번호 검증 로직을 건너뛰도록 조건부 로직 구현.
...
개발 환경(Vite): vite.config.ts의 proxy 설정을 통해 /api 요청을 로컬 백엔드로 우회.

배포 환경: SecurityConfig에서 AllowedOrigins를 명시적으로 허용하고, WebMvcConfigurer를 통해 전역 CORS 정책 설정.

6. ERD (Entity Relationship Diagram)
(※ 실제 ERD 이미지가 없다면 아래처럼 텍스트로 요약해도 좋습니다)
...
ProgramSchedules와 N:1 관계 (프로그램 예약 시)
...
backend/src/main/resources/application.properties의 DB 설정을 본인 환경에 맞게 수정합니다.
